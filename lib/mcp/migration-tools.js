...
const MIGRATION_TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description: '[WRITE OPERATION — requires dryRun=false for live execution. User must hold valid SAP Named User license.] Run ETLV (Extract-Transform-Load-Validate) for a single migration object. dryRun=true by default.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: '[WRITE OPERATION — requires dryRun=false for live execution. User must hold valid SAP Named User license.] Run ETLV for all migration objects in dependency order. dryRun=true by default.',
    inputSchema: {
      type: 'object',
      properties: {
        dryRun: { type: 'boolean', description: 'If true, simulate without loading to target', default: true },
