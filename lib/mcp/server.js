 ...
 * SDK dependencies.
 *
 * Supports 108 SAP tools and 4 resource types in both mock and live modes.
 */
...
const { FORENSIC_TOOL_DEFINITIONS } = require('./forensic-tools');
const { ASSESSMENT_TOOL_DEFINITIONS } = require('./assessment-tools');
const { MIGRATION_TOOL_DEFINITIONS } = require('./migration-tools');
const { GREENFIELD_TOOL_DEFINITIONS } = require('./greenfield-tools');
const { CONNECTION_TOOL_DEFINITIONS } = require('./connection-tools');
const { CLOUD_TOOL_DEFINITIONS } = require('./cloud-tools');
const { CONFIG_TOOL_DEFINITIONS } = require('./config-tools');
const SessionContext = require('./session-context');

const PROTOCOL_VERSION = '2024-11-05';
const SERVER_NAME = 'sen-mcp-server';
const SERVER_VERSION = '1.0.0';
const SERVER_COMPLIANCE = {
  sapLicenseRequired: true,
  sapLicenseTypes: ['Named User'],
  apisUsed: ['OData V2', 'OData V4', 'RFC', 'ADT REST'],
  dataStorage: 'none-persistent',
  credentialHandling: 'in-memory-only',
  writeOperationDefault: 'dry-run',
  auditTrail: true
};
...
const TOOL_DEFINITIONS = [
  ...
  {
    name: 'writeSource',
    description: '[WRITE OPERATION — requires dryRun=false for live execution. User must hold valid SAP Named User license.] Write ABAP source code to a given object URI. Performs lock, write, unlock sequence.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: '[WRITE OPERATION — requires dryRun=false for live execution. User must hold valid SAP Named User license.] Create, release, or query transport requests.',
    inputSchema: {
      type: 'object',
      properties: {
...
const MOCK_DATA = {
  ...
  signavio_parse_process: (_params) => ({
  ...
  signavio_map_to_config: (_params) => ({
    processName: 'Order-to-Cash',
    configSteps: [
  ...
  test_generate_from_bpmn: (_params) => ({
    testScenarios: [
...
class McpServer {
  ...
  async _handleToolsList() {
    const allTools = [
      ...TOOL_DEFINITIONS, ...INFOR_TOOL_DEFINITIONS,
      ...FORENSIC_TOOL_DEFINITIONS, ...ASSESSMENT_TOOL_DEFINITIONS,
      ...MIGRATION_TOOL_DEFINITIONS, ...GREENFIELD_TOOL_DEFINITIONS,
      ...CONNECTION_TOOL_DEFINITIONS, ...CLOUD_TOOL_DEFINITIONS,
  ...
  async _handleToolsCall(name, args) {
    ...
    // Universal bridge tool handlers — lazy-initialized per domain
    const HANDLER_MAP = {
      forensic_:   { prop: '_forensicHandlers',   Cls: () => require('./forensic-tool-handlers').ForensicToolHandlers },
      assessment_: { prop: '_assessmentHandlers', Cls: () => require('./assessment-tool-handlers').AssessmentToolHandlers },
      migration_:  { prop: '_migrationHandlers',  Cls: () => require('./migration-tool-handlers').MigrationToolHandlers },
      greenfield_: { prop: '_greenfieldHandlers', Cls: () => require('./greenfield-tool-handlers').GreenfieldToolHandlers },
      connection_: { prop: '_connectionHandlers', Cls: () => require('./connection-tool-handlers').ConnectionToolHandlers },
      cloud_:      { prop: '_cloudHandlers',      Cls: () => require('./cloud-tool-handlers').CloudToolHandlers },
      config_:     { prop: '_configHandlers',     Cls: () => require('./config-tool-handlers').ConfigToolHandlers },
    };
    for (const [prefix, { prop, Cls }] of Object.entries(HANDLER_MAP)) {
      if (name.startsWith(prefix)) {
        if (!this[prop]) {
          this[prop] = new (Cls())({ mode: this.mode, sessionContext: this._sessionContext, logger: this.log });
        }
        const result = await this[prop].handle(name, args);
        return {
          content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
        };
      }
