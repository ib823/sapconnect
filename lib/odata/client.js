...
class ODataClient {
  ...
  constructor(options = {}) {
    ...
    this.sapClient = options.sapClient || null; // SAP client (mandant) e.g. '100'
  ...
  /**
   * Fetch and parse $metadata into a structured model.
   * Requires MetadataParser (lazy-loaded).
   * @param {string} servicePath
   * @returns {object} Parsed metadata model with entityTypes, entitySets, etc.
   */
  async parseMetadata(servicePath) {
    const MetadataParser = require('./metadata-parser');
    const parser = new MetadataParser({ logger: this.log });
    const xml = await this.getMetadata(servicePath);
  ...
  _buildUrl(path, params = {}) {
    ...
    // Append sap-client if configured and not already present
    if (this.sapClient && !url.searchParams.has('sap-client')) {
      url.searchParams.set('sap-client', this.sapClient);
    }
  ...
  async _execute(method, url, body, extraHeaders = {}, rawResponse = false) {
    ...
    let response;
    try {
      response = await this._circuitBreaker.execute(() => fetch(url, fetchOptions));
    } catch (err) {
      if (err.name === 'CircuitBreakerOpenError' || err.code === 'CIRCUIT_OPEN') {
        throw new ConnectionError(`Circuit breaker open: ${err.message}`, { url, method });
      }
