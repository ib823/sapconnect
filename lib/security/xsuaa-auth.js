...
class XsuaaAuth {
  ...
  _getCredentials() {
    ...
    if (process.env.VCAP_SERVICES) {
      const parsed = XsuaaAuth._extractCredentialsFromVcap(process.env.VCAP_SERVICES);
      if (parsed) {
        this._credentials = parsed;
        return this._credentials;
      }
    }

    // Compatibility path for environments that still provide @sap/xsenv.
    try {
  ...
  middleware() {
    return (req, res, next) => {
      ...
      const initialize = typeof this._passport.initialize === 'function'
        ? this._passport.initialize()
        : (_req, _res, initializeNext) => initializeNext();

      initialize(req, res, (initErr) => {
        if (initErr) {
          this.log.error('XSUAA passport initialization error', { error: initErr.message });
          return res.status(500).json({ error: 'Authentication initialization error' });
        }
  ...
  static _extractCredentialsFromVcap(rawVcapServices) {
    const services = typeof rawVcapServices === 'string'
      ? JSON.parse(rawVcapServices)
      : rawVcapServices;

    const bindings = [];
    for (const serviceGroup of Object.values(services || {})) {
      if (Array.isArray(serviceGroup)) {
        bindings.push(...serviceGroup);
      }
    }
    ...
    const binding = bindings.find((candidate) => {
      ...
      return label === 'xsuaa' || name?.includes('xsuaa') || tags.includes('xsuaa');
    });
