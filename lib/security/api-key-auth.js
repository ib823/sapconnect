 ...
 * When API_KEY is configured: requires X-API-Key header.
 * When not configured in development: passthrough (dev mode).
 * In production (NODE_ENV=production): requires API_KEY — fails fast otherwise.
...
class ApiKeyAuth {
  ...
  constructor(options = {}) {
    ...
    if (process.env.NODE_ENV === 'production' && !this.isEnabled()) {
      throw new Error(
        'API_KEY must be configured in production. ' +
        'Set the API_KEY environment variable or use AUTH_STRATEGY=xsuaa for BTP deployments.'
      );
    }
  }
