const ObjectClass = require('../../../migration/objects/employee-master');
...
describe('Employee Master', () => {
  ...
  it('has correct objectId and name', () => {
    expect(obj.objectId).toBe('EMPLOYEE_MASTER');
    expect(obj.name).toBe('Employee Master');
  });

  it('has 54 field mappings', () => {
    expect(obj.getFieldMappings()).toHaveLength(54);
  });

  it('extracts 40 mock records', async () => {
    const result = await obj.run();
    expect(result.phases.extract.recordCount).toBe(40);
  });
  ...
  it('validates required fields', () => {
    ...
    expect(checks.required).toEqual(['PersonnelNumber', 'LastName', 'FirstName', 'CompanyCode']);
  });

  it('checks for duplicates on correct keys', () => {
    const checks = obj.getQualityChecks();
    expect(checks.exactDuplicate.keys).toEqual(['PersonnelNumber']);
  });
