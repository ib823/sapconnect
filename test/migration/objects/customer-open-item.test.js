const ObjectClass = require('../../../migration/objects/customer-open-item');

describe('Customer Open Items', () => {
  const gw = { mode: 'mock' };
  let obj;
  ...
  it('has correct objectId and name', () => {
    expect(obj.objectId).toBe('CUSTOMER_OPEN_ITEM');
    expect(obj.name).toBe('Customer Open Items');
  });
  ...
  it('validates required fields', () => {
    ...
    expect(checks.required).toEqual(['CompanyCode', 'Customer', 'DocumentNumber', 'FiscalYear', 'AmountInCompanyCodeCurrency']);
  });
